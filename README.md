...
## How?
The script was meant to work by running each time the power managment directory is updated in the Library folder of the OS but that didn't work so I updated the plist file to run every 600s. It starts off by checking if the laptop is conected to a power source, if so it disables LPM and stops, the second check occurs if its not connected to AC Power and checks if LPM is on, if so it stops running the script. The third check consists of two different conditions, if the battery level is between 30% and 10% (and lowpowermode is off), it enables LPM and notifies the user with a password prompt, if the battery level is less than 10%, then is shows another notification prompting the user to connect to AC Power. 

## Set-Up
1. Download all files, place the .scpt anywhere and place the .plist file in /Library/LaunchAgents
2. export the script as an executable in Script Editor
...
3. Figure out how to make it run automatically after battery level changes
